...
현재 위치와 남은 길을 한 줄에 정리하면:



**토요일 07:00 이후 (확인만):**
1. 자동 실행 결과 확인 — Actions 초록불 + 로그의 `op_e` 숫자, 또는 `data/m7_revision/` CSV
2. 나한테 보고 → `op_e 250+`면 **m7 완전 종결**, op=0이면 perf-diag 지문으로 확정 수정 1회

**m7 종결 후 — 파인 작업 (이번 주말~):**
3. **층1 레짐 대시보드** (오버레이형 확정해둔 그것): 주식-금리 롤링 상관 부호 + FX 게이트 + 신용스프레드를 배경색 레짐 띠 + 구석 상태 테이블로. 형님 기존 차트에 얹는 방식
4. **업종 모멘텀 스크리너** (파인): TV의 KRX 업종지수 12개월 모멘텀 — "지금 가는 분야" 판독기. 이게 형님이 처음 던진 질문("가는 분야를 알면 스캔 범위가 좁아진다")의 첫 실물이 된다

**알아서 돌아가는 것 (형님 손 불필요):**
- m7 로봇: 매주 토 07:00 자동. 월 1회 초록불만 확인
- **4주 뒤(8월 초)**: 스냅샷 4개가 쌓이는 시점 — 그때 Colab 리비전 분석 노트북(Δ4주 계산 → 업종별 집계)을 만든다. 그 전엔 만들어도 계산할 데이터가 없다

즉 형님의 다음 능동 작업은 **4번(파인 대시보드)**이고, 그 시작 시점은 형님이 정하면 된다 — 아침 m7 판정 보고 시작할지, 지금 바로 스펙부터 잡을지. 어느 쪽?
